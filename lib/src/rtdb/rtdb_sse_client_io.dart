...
class RtdbSseClient {
  ...
  RtdbSseClient(this.baseUri, {HttpClient? httpClient})
  ...
  Stream<RtdbSseEvent> listen(
    ...
    () async {
      ...
      if (response.statusCode != HttpStatus.ok) {
        ...
        controller.addError(
          HttpException(
              'RTDB SSE failed: ${response.statusCode} ${response.reasonPhrase}. $body'),
        );
  ...
  void _handleEventBlock(
      String rawEvent, StreamController<RtdbSseEvent> controller) {
    String? event;
    ...
    controller.add(toRtdbEvent(event, rawData));
  }
}
